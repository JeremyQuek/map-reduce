/**
 * The MapReduce coordinator.
 */

#include "coordinator.h"
#include "job.h"

#ifndef SIG_PF
#define SIG_PF void (*)(int)
#endif

/* Global coordinator state. */
coordinator* state;

extern void coordinator_1(struct svc_req*, SVCXPRT*);

/* Set up and run RPC server. */
int main(int argc, char** argv) {
  register SVCXPRT* transp;

  pmap_unset(COORDINATOR, COORDINATOR_V1);

  transp = svcudp_create(RPC_ANYSOCK);
  if (transp == NULL) {
    fprintf(stderr, "%s", "cannot create udp service.");
    exit(1);
  }
  if (!svc_register(transp, COORDINATOR, COORDINATOR_V1, coordinator_1, IPPROTO_UDP)) {
    fprintf(stderr, "%s", "unable to register (COORDINATOR, COORDINATOR_V1, udp).");
    exit(1);
  }

  transp = svctcp_create(RPC_ANYSOCK, 0, 0);
  if (transp == NULL) {
    fprintf(stderr, "%s", "cannot create tcp service.");
    exit(1);
  }
  if (!svc_register(transp, COORDINATOR, COORDINATOR_V1, coordinator_1, IPPROTO_TCP)) {
    fprintf(stderr, "%s", "unable to register (COORDINATOR, COORDINATOR_V1, tcp).");
    exit(1);
  }

  coordinator_init(&state);

  svc_run();
  fprintf(stderr, "%s", "svc_run returned");
  exit(1);
  /* NOTREACHED */
}

/* EXAMPLE RPC implementation. */
int* example_1_svc(int* argp, struct svc_req* rqstp) {
  static int result;

  result = *argp + 1;

  return &result;
}

/* SUBMIT_JOB RPC implementation. */
int* submit_job_1_svc(submit_job_request* argp, struct svc_req* rqstp) {
  static int result;

  printf("Received submit job request\n");

  /* TODO */
  
  /* Do not modify the following code. */
  /* BEGIN */
  struct stat st;
  if (stat(argp->output_dir, &st) == -1) {
    mkdirp(argp->output_dir);
  }

  // validate app
  if (get_app(argp->app).map == NULL) {
    result = -1;
    return &result;
  }
  job_t* job = job_create(argp, state->next_job_id++);
  g_queue_push_tail(state->job_queue, job);

  result = job->job_id;
  return &result;
  /* END */
}

/* POLL_JOB RPC implementation. */
poll_job_reply* poll_job_1_svc(int* argp, struct svc_req* rqstp) {
  static poll_job_reply result;

  // printf("Received poll job request\n");

  /* TODO */
  int job_id = *argp;
  for (GList* elem = state->job_queue->head; elem; elem = elem->next) {
    job_t* job = (job_t*)elem->data;
    if (job->job_id == job_id) {
      result.invalid_job_id = false;
      result.done = (job->status == JOB_DONE || job->status == JOB_FAILED);
      result.failed = (job->status == JOB_FAILED);
      return &result;
    }
  }

  result.invalid_job_id = true;
  result.done = false;
  result.failed = false;
  return &result;
}

/* GET_TASK RPC implementation. */
get_task_reply* get_task_1_svc(void* argp, struct svc_req* rqstp) {
  static get_task_reply result;

  // printf("Received get task request\n");
  result.file = "";
  result.output_dir = "";
  result.app = "";
  result.wait = true;
  result.args.args_len = 0;

  /* TODO */
  for (GList* elem = state->job_queue->head; elem; elem = elem->next) {
    job_t* job = (job_t*)elem->data;
    if (job->status == JOB_DONE || job->status == JOB_FAILED) continue;

    if (job->status == JOB_MAPPING) {
      update_map_task_timeout(job);
      for (int i = 0; i < job->n_map; i++) {
        if (job->map_tasks[i] == 0) {
          job->map_tasks[i] = time(NULL);
          result.wait = false;
          result.reduce = false;
          result.task = i;
          result.file = job->files[i];
          result.output_dir = job->output_dir;
          result.app = job->app;
          result.job_id = job->job_id;
          result.n_reduce = job->n_reduce;
          result.n_map = job->n_map;
          result.args.args_val = job->args;
          result.args.args_len = job->args_len;

          printf("Received get task request, assign map task: %d of job %d\n", i, job->job_id);
          return &result;
        }
      }
    }
    else {
      update_reduce_task_timeout(job);
      for (int i = 0; i < job->n_reduce; i++) {
        if (job->reduce_tasks[i] == 0) {
          job->map_tasks[i] = time(NULL);
          result.wait = false;
          result.reduce = true;
          result.task = i;
          result.file = "";
          result.output_dir = job->output_dir;
          result.app = job->app;
          result.job_id = job->job_id;
          result.n_reduce = job->n_reduce;
          result.n_map = job->n_map;
          result.args.args_val = job->args;
          result.args.args_len = job->args_len;

          printf("Received get task request, assign reduce task: %d of job %d\n", i, job->job_id);
          return &result;
        }
      }
    }
  }
  printf("Received get task request, no job to assign yet\n");
  return &result;  
}

/* FINISH_TASK RPC implementation. */void* finish_task_1_svc(finish_task_request* argp, struct svc_req* rqstp) {
  static char* result;

  for (GList* elem = state->job_queue->head; elem; elem = elem->next) {
    job_t* job = (job_t*)elem->data;
    if (job->job_id != argp->job_id) continue;

    if (!argp->success) {
      job->status = JOB_FAILED;
      break;
    }

    if (argp->reduce) {
      job->reduce_tasks[argp->task] = -1;
      if (job_all_reduces_done(job)) job->status = JOB_DONE;
    } else {
      job->map_tasks[argp->task] = -1;
      if (job_all_maps_done(job)) job->status = JOB_REDUCING;
    }
    break;
  }
  return (void*)&result;
}

/* Initialize coordinator state. */
void coordinator_init(coordinator** coord_ptr) {
  *coord_ptr = malloc(sizeof(coordinator));
  coordinator* coord = *coord_ptr;

  coord->next_job_id = 0;
  coord->job_queue = g_queue_new();
}
